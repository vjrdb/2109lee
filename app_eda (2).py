...
class PopulationTrendsEDA:
    def __init__(self):
        ...
        # 2. 연도별 추이
        with tabs[1]:
            st.header("📆 Yearly Population Trends")
            national_df = df[df['지역'] == '전국']
            plt.figure(figsize=(10, 4))
            sns.lineplot(x='연도', y='인구', data=national_df)
            plt.title("Total Population Over Years")
            plt.xlabel("Year")
            plt.ylabel("Population")

            # 인구 예측 (단순 추세선 기반)
            recent = national_df.sort_values('연도').tail(3)
            births = recent['출생아수(명)'].mean()
            deaths = recent['사망자수(명)'].mean()
            population_2035 = national_df['인구'].iloc[-1] + (births - deaths) * (2035 - national_df['연도'].iloc[-1])
            plt.scatter(2035, population_2035, color='red')
            plt.text(2035, population_2035, f"2035 pred: {int(population_2035):,}", fontsize=9)
            st.pyplot(plt)

        # 3. 지역별 분석
        with tabs[2]:
            st.header("📍 Regional Changes")
            df = df[df['지역'] != '전국']
            latest_year = df['연도'].max()
            five_years_ago = latest_year - 5

            df_5 = df[df['연도'].isin([five_years_ago, latest_year])]
            pivot_df = df_5.pivot(index='지역', columns='연도', values='인구')
            pivot_df['change'] = (pivot_df[latest_year] - pivot_df[five_years_ago]) / 1000
            pivot_df['change_pct'] = (pivot_df[latest_year] - pivot_df[five_years_ago]) / pivot_df[five_years_ago] * 100
            pivot_df = pivot_df.sort_values('change', ascending=False)

            # 수평 막대그래프 (변화량)
            fig, ax = plt.subplots()
            sns.barplot(x='change', y=pivot_df.index, data=pivot_df, ax=ax)
            ax.set_title("Change in Population (Last 5 Years)")
            ax.set_xlabel("Change (thousands)")
            for i, v in enumerate(pivot_df['change']):
                ax.text(v, i, f"{v:.1f}", va='center')
            st.pyplot(fig)

            # 변화율 그래프
            fig2, ax2 = plt.subplots()
            sns.barplot(x='change_pct', y=pivot_df.index, data=pivot_df, ax=ax2)
            ax2.set_title("Change Rate (%)")
            ax2.set_xlabel("Rate of Change")
            for i, v in enumerate(pivot_df['change_pct']):
        ...
        with tabs[3]:
            ...
            top100 = df_sorted[df_sorted['지역'] != '전국'].nlargest(100, 'diff')

            st.dataframe(
                top100.style.background_gradient(
                    subset=['diff'], cmap='coolwarm', axis=0).format({
                    '인구': "{:,}", 'diff': "{:,}"
                })
            )
        ...
        with tabs[4]:
            ...
            pivot = df.pivot_table(index='연도', columns='지역', values='인구', aggfunc='sum')
